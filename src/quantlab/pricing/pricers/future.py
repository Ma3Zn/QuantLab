...
class FuturePricer:
    """Price linear futures as mark-to-market notionals."""
    ...
    def price(
        ...
        *,
        ...
        if instrument_currency is None:
            raise ValueError("Future instruments must declare a currency")

        market_data_id = instrument.market_data_id
        if market_data_id is None:
            raise ValueError("Future instruments must declare a market_data_id")

        if not isinstance(instrument.spec, FutureSpec):
            raise ValueError("FuturePricer requires FutureSpec instruments")
        ...
        notional_native = position.quantity * unit_price * instrument.spec.multiplier
